...
define([
    ...
    return function (config) {
        var selectedCategory = config.selectedCategories,
        selectedCategory = $H(selectedCategory),
        gridJsObject = window[config.gridJsObjectName],
        tabIndex = 1000;
        console.log(gridJsObject); 
        $("in_categories").value = Object.toJSON(selectedCategory);
         ...
         * @param {String} row
         */
        function selectedProductRowInit(grid, row, index)
        {
            ...
            if (checkbox && position) {
                ...
                /*  if(tempSelectedProduct[index+1]!=undefined)
                {
                    position.value = tempSelectedProduct[index+1];
                }*/
        ...
        if (gridJsObject.rows) {
            gridJsObject.rows.each(function (row,index) {
                selectedProductRowInit(gridJsObject, row, index);
            });
        }
    };
});
